...
RegisterNetEvent('moonshine:server:StartJob', function()
  ...
  if playerCooldowns[src] and now < playerCooldowns[src] then
    ...
    TriggerClientEvent('ox_lib:notify', src, { description = "Wait "..rem.." more minute(s).", type = "error" })  -- :contentReference[oaicite:0]{index=0}
    return
  end
  ...
  if policeCount < Config.MinPolice then
    TriggerClientEvent('ox_lib:notify', src, { description = "Not enough police on duty.", type = "error" })  -- :contentReference[oaicite:2]{index=2}
    return
  end
...
RegisterNetEvent('moonshine:server:Spill', function(amount)
  ...
  if spill > 0 then
    ...
    TriggerClientEvent('ox_lib:notify', src, { description = spill.." bottles spilled!", type = "error" })  -- :contentReference[oaicite:5]{index=5}
  end
end)
...
RegisterNetEvent('moonshine:server:CompleteJob', function()
  ...
  TriggerClientEvent('ox_lib:notify', src, {
    description = ("Delivered %d bottles for $%d black money."):format(job.bottles, payout),
    type        = "success"
